...
export async function runGeminiSearch(
  ...
  try {
    ...
    const disallowedToolEvents = telemetry.toolUses.filter((event) => event.tool_name !== "google_web_search");
    const policyWarnings = [];
    if (disallowedToolEvents.length > 0) {
      policyWarnings.push(`Detected ${disallowedToolEvents.length} non-web tool event(s).`);
    }
      ...
      return {
        ...
        details: {
          ...
          progress: {
            ...
            otherToolCalls: progress.otherToolCalls
          }
        }
      };
    }

    if (failOnCommandEvent && disallowedToolEvents.length > 0) {
      return {
        ok: false,
    ...
    const content = [
      ...
      `- searches: ${progress.searches}`,
      `- other tools: ${progress.otherToolCalls}`
    ];

    if (structured.notes) content.push("", `Notes: ${structured.notes}`);
    if (policyWarnings.length) content.push("", `Warnings: ${policyWarnings.join(" ")}`);
